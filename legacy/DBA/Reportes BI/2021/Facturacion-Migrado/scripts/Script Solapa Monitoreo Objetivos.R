...
#"E:/Personales/Sistemas/Agustin/Reportes BI/2021/Facturacion-Migrado/excels"

#WorkDirectoryComprobantesDesestimar <- "C:/Users/iachenbach/Gobierno de la Ciudad de Buenos Aires/Pablo Alfredo Gadea - Tablero Facoep P BI/FACOEP/DBA/Reportes BI/2021/Facturaci?n"
...
SIF2Query <- glue(paste("SELECT pprnombre,",
                        ...
                        "comprobantepprid > 0 AND",
                        "c.tipocomprobantecodigo IN({comprobantes_query}) AND",
                        "c.comprobanteccosto = 5 AND",
                        "c.comprobantefechaemision > '2017-12-31'"),sep = "\n")
...
SIF2 <- aggregate(SIF2$comprobantecrgimportetotaldeta
                  ,by = list(SIF2$EfectorObjetivos,SIF2$AnioEmision,SIF2$MostrarGrafico),
                  FUN = sum)
