...
junio <- dbConnect(drv, dbname = "facoep30092022", host = "172.31.24.12", port = 5432, user = "postgres", password = pw)
mayo <- dbConnect(drv, dbname = "facoep31082022", host = "172.31.24.12", port = 5432, user = "postgres", password = pw)
...
CASE WHEN comprobantepprid = 3140 THEN 'Turismo'
     ...
     entrega, vencimiento,
 CASE WHEN vencimiento < '31-08-2022' THEN EXTRACT(YEAR FROM vencimiento)::varchar
      WHEN EXTRACT(MONTH FROM vencimiento) = 9 THEN 'Actual'
      WHEN vencimiento >= '01-10-2022' THEN 'A Vencer' ELSE '' END AS Periodo,
...
Mandatarios06 <-  dbGetQuery(junio, "SELECT
                          ...
                          WHERE cg.tipocomprobantecodigo in ('FACA2','FACB2','FAECA','FAECB','NDA', 'NDB', 'NDECA') and
                                EXTRACT(YEAR FROM ec.enviocompfecha) = 2022 and EXTRACT(MONTH FROM ec.enviocompfecha) = 9
...
Recibos <- dbGetQuery(junio, "SELECT
      ...
      WHEN vencimiento >= '01-10-2022' THEN 'A Vencer' ELSE '' END AS Periodo,
                          ...
                          WHERE c.tipocomprobantecodigo in ('RECX2') and ci.comprobanteimputaciontipo in ('FACA2','FACB2','FAECA','FAECB','NDA', 'NDB', 'NDECA') and
                                EXTRACT(YEAR FROM c.comprobantefechaemision) = 2022 and EXTRACT(MONTH FROM c.comprobantefechaemision) = 09 
                                --and c.comprobanteentidadcodigo = 270
...
write.csv(Reporte,"Reporte Cobranzas Octubre.csv")
