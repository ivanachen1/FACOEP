...
NotasDebito <- TransformFile(tabla_parametros_comprobantes,FilterOne = "notadebito")
...
ComprobantesAux <- dbGetQuery(conn = con,
                              glue(paste("SELECT CONCAT(tipocomprobantecodigo,'-',comprobanteprefijo,'-',comprobantecodigo) as notacredito,",
                                         "CASE WHEN comprobantedetalle LIKE '%ANULADO%' THEN 'SI' ELSE 'NO' END AS NdAnulada,",
                                         "CASE WHEN comprobantedetalle LIKE '%intereses%' THEN 'SI'",
                                              "WHEN comprobantedetalle LIKE '%Intereses%' THEN 'SI' ELSE 'NO' END AS NdInteres,",
...
ftinteres <- data.frame(table("interes" = ComprobantesAux$ndinteres,"anulada" = ComprobantesAux$ndanulada))

importesND <- aggregate(ComprobantesAux$importenc, by=list(Interes=ComprobantesAux$ndinteres,Anulada = ComprobantesAux$ndanulada), FUN=sum)

# tomar las notas de debito
# Mirar los crgs en cero

# Ver el caso de la NDB-1-1541
# Ver el caso de la NDB-1-2349
